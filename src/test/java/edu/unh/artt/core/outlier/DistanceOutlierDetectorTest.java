            ...
            for(int var : variances) {
                for(long mean : means) {
                    for(int modes = 1; modes < 4; modes++) {
                        ...
                        assertTrue(outlierDetector.isOutlier(new OffsetGmSample(0,0, 1000 * var + mean, new byte[8])));
                        assertTrue(outlierDetector.isOutlier(new OffsetGmSample(0,0, Integer.MAX_VALUE, new byte[8])));
                        assertTrue(outlierDetector.isOutlier(new OffsetGmSample(0,0, -3000 * var + mean, new byte[8])));
                        assertTrue(outlierDetector.isOutlier(new OffsetGmSample(0,0, Long.MIN_VALUE >> 16, new byte[8])));

                        assertFalse(outlierDetector.isOutlier(new OffsetGmSample(0,0, mean, new byte[8])));
                        assertFalse(outlierDetector.isOutlier(new OffsetGmSample(0,0, mean + var, new byte[8])));
                        assertFalse(outlierDetector.isOutlier(new OffsetGmSample(0,0, mean - var, new byte[8])));
                    }
                }
            }
